  ...
  <div class="trial-booking-page frontend-layout">
    <div class="page-content section-container">
      <h2 class="section-title">
        預約<span class="text-highlight">體驗課程</span>
      </h2>
      ...
      <div v-else class="trial-list">
        ...
        >
          ...
          <el-table-column label="體驗名額" width="120">
            <template #default="{ row }">
              {{ row.bookedTrialCount || 0 }} /
              {{ row.maxTrialCapacity || row.maxCapacity }}
            </template>
          </el-table-column>
          <el-table-column label="操作">
            <template #default="{ row }">
              <el-button
                ...
                :disabled="
                  row.hasBookedTrial ||
                  row.bookedTrialCount >= row.maxTrialCapacity
                "
              >
...
const apiBaseUrl =
  import.meta.env.VUE_APP_API_URL || "http://localhost:8080/api";
...
const bookingRules = reactive({
  ...
  time: [
    { required: true, message: "請選擇預約日期與時間", trigger: "change" },
  ],
});
...
const fetchTrialCourses = async () => {
  ...
  try {
    ...
    const response = await axios.get(`${apiBaseUrl}/courses/trial`, {
    ...
    const errorMessage =
      error.response?.data?.message || "無法載入體驗課程列表。";
  ...
  bookingFormRef.value.validate(async (valid) => {
    if (valid) {
      ...
      try {
        ...
        const errorMessage =
          error.response?.data?.message || "預約體驗課程失敗，請稍後再試。";
        ElMessage.error(errorMessage);
        isLoading.value = false;
      } finally {
