    ...
    <div class="health-platform dark-theme">
      ...
      <section class="courses-section" id="courses">
        <div class="section-container">
          ...
          <div
            v-else-if="hotCourses?.length === 0"
            class="empty-list-placeholder"
          >
            <el-empty description="目前沒有熱門課程"></el-empty>
          </div>
                  ...
                  <img
                    :src="
                      course.images && course.images.length > 0
                        ...
                        : 'https://placehold.co/400x250/cccccc/ffffff?text=No+Image'
                    "
                    :alt="course.name"
                    @error="
                      (event) =>
          ...
          <div class="view-all-courses">
            <router-link to="/courses" class="view-all-courses-link">
      ...
      <section class="trainers-section" id="trainers">
        <div class="section-container">
          <h2 class="section-title">
            專業 <span class="text-highlight">教練團隊</span>
          </h2>
      ...
      <section class="features-section" id="features">
        <div class="section-container">
          ...
          <div class="feature-cards">
            <router-link
              to="/user/fitness?tab=diet-records"
              class="feature-link"
            >
            ...
            <router-link
              to="/user/fitness?tab=workout-records"
              class="feature-link"
            >
              <div class="feature-card">
                <div class="feature-icon">
            ...
            <router-link
              to="/user/fitness?tab=goals-progress"
              class="feature-link"
            >
              <div class="feature-card">
                <div class="feature-icon">
        ...
        <div class="section-container">
          ...
          <div class="dashboard-container">
            <div class="dashboard-info">
              ...
              <el-button type="success" @click="viewDemoData"
                >查看示範數據</el-button
              >
            </div>
      ...
      <section class="community-section" id="social">
        <div class="section-container">
          <h2 class="section-title">
            加入健康 <span class="text-highlight">社區</span>
          </h2>
          <div class="community-content">
            <div class="community-info">
              <p>
                與志同道合的健康夥伴分享心得、交流經驗，一起邁向更健康的生活。
              </p>
      ...
      <section class="testimonial-section">
        <div class="section-container">
          <h2 class="section-title">
            會員 <span class="text-highlight">評價</span>
          </h2>
      ...
      <section class="faq-section" id="faq">
        <div class="section-container">
          <h2 class="section-title">
            常見 <span class="text-highlight">問題</span>
          </h2>
...
import trainer4Image from "@/assets/images/trainer4.jpg";
import {
  ElMessage,
  ElSkeleton,
  ElEmpty,
  ...
  ElCol,
  ElButton,
  ElCollapse, // 確保 ElCollapse 被引入
  ElCollapseItem, // 確保 ElCollapseItem 被引入
  ElDialog, // 確保 ElDialog 被引入
  ElForm, // 確保 ElForm 被引入
  ElFormItem, // 確保 ElFormItem 被引入
  ElInput, // 確保 ElInput 被引入
  ElDatePicker, // 確保 ElDatePicker 被引入
} from "element-plus";
import { getProducts, addItemToCart } from "@/api/shop";
// 刪除這一行，Sidebar 功能已經移到 Navbar
...
export default {
  ...
  components: {
    ...
    ElEmpty,
    ArrowRight,
    // 確保這些 Elment Plus 組件有被註冊 (如果您在 setup 中直接使用它們的標籤)
    ElCollapse,
    ElCollapseItem,
    ElDialog,
    ElForm,
  ...
  setup() {
    ...
    // 用於存放按報名人數排序的熱門課程
    const allCourses = ref([]); // 用於存放從後端獲取的所有課程

    const fetchCoursesAndSetHot = async () => {
      isLoading.value = true;
      try {
        const response = await axios.get("/api/courses"); // 獲取所有課程 (包含人數信息)
        ...
        if (response.data && Array.isArray(response.data)) {
          ...
          console.warn("獲取課程數據格式異常:", response.data);
          hotCourses.value = []; // 如果數據格式不對，清空熱門課程
          isLoading.value = false;
          return; // 數據異常時直接返回
        }

        if (coursesData.length > 0) {
          // 根據 registeredCount 排序並取出**前 3 或 4 名**
          hotCourses.value = [...coursesData]
            .sort((a, b) => {
              const countA = a.registeredCount || 0;
              const countB = b.registeredCount || 0;
              return countB - countA; // 降序排序
            })
            .slice(0, 4); // === 改為取前 4 個 (可以調整數量) ===
          console.log(
            "獲取並設置熱門課程成功:",
            hotCourses.value.length,
            "個課程"
          );
        } else {
          hotCourses.value = []; // 如果沒有課程數據，清空熱門課程
          console.log("沒有獲取到課程數據，熱門課程為空");
        }
      } catch (error) {
        console.error("獲取課程失敗:", error);
        ElMessage.error("無法載入課程，請稍後再試！");
        hotCourses.value = []; // 載入失敗時清空熱門課程
      } finally {
        isLoading.value = false; // 所有數據載入結束後設置為 false
      }
    };

    // 修改 onMounted (保留，但移除 async await 如果裡面已經沒有 await)
    onMounted(async () => {
      // 這裡保留 async 是因為 Google OAuth 邏輯需要 async
      console.log("HomePage mounted.");

      // 將 fetch 數據的邏輯移到這裡並使用 await
      isLoading.value = true;
      await Promise.all([fetchCoursesAndSetHot(), fetchPopularProducts()]);
      isLoading.value = false;

      // ... 保留處理 Google OAuth 登入重定向後的 URL 參數的邏輯 ...
      // 檢查 URL 中是否有 token 参数（Google OAuth 登录重定向后）
      ...
      if (token) {
        ...
        // 配置 axios 默认请求头
        // import("axios").then(({ default: axios }) => { // 這個 import 移到檔案頂部
    ...
    const recentPosts = ref([
      {
        ...
        excerpt:
          "堅持運動三個月，我不僅瘦了10公斤，更重要的是養成了健康的生活習慣...",
        author: "小明",
        date: "3天前",
        comments: 15,
      },
    ...
    const popularCourses = ref([
      ...
      },
      {
        ...
        image:
          "https://via.placeholder.com/300x200/2196F3/FFFFFF?Text=Strength",
        coach: "王教練",
        duration: "90 分鐘",
      },
    ]);
    const trainers = ref([
      {
        ...
        photo: trainer1Image, // <-- 修改這裡，使用匯入的變數
    ...
    const faqData = ref([
      {
        ...
        answer:
          "您只需要註冊一個帳號，即可開始使用我們的各項功能，包括飲食追蹤、運動記錄、身體指標監測和目標設定。",
      },
      {
        id: 402,
        question: "平台上的健康建議是個人化的嗎？",
        answer:
          "是的，我們的系統會根據您輸入的個人資料和健康數據，提供個人化的飲食和運動建議。",
      },
      {
        id: 403,
        question: "如何聯繫專業教練？",
        answer:
          "您可以在教練團隊頁面查看教練的個人資料和聯繫方式，或者透過平台內的訊息系統與教練溝通。",
      },
      {
        id: 404,
        question: "平台是否提供免費試用？",
        answer:
          "我們提供新用戶一定的免費體驗期，讓您體驗平台的核心功能。詳細資訊請查看我們的訂閱方案。",
      },
    ]);
    const activeFaq = ref([]);
    ...
    const getProductImageUrl = (product) => {
      if (!product)
        return "https://placehold.co/400x300/4CAF50/ffffff?text=Product";
      ...
      if (product.category) {
        ...
        const color =
          categoryColors[product.category.toLowerCase()] || "4CAF50";
    ...
    // 在 onMounted 中处理 Google OAuth 登录重定向后的 URL 参数 (保留)
    onMounted(async () => {
      // 這裡保留 async 是因為 fetchCoursesAndSetHot 等是 async
      console.log("HomePage mounted.");
    ...
    // 此處 bookTrainer 如有使用，可自行定義邏輯 (保留)
    ...
    // setup 函數的返回 (移除 sidebar 相關的，保留其他的)
    return {
      // isSidebarOpen, // 移除
      // toggleSidebar, // 移除
      subscribeEmail,
      ...
      bookTrainer,
      viewProductDetail,
      getDefaultProductImage, // 如果在 setup 中沒有直接調用，可以不返回
      getProductImageUrl,
      displayProducts,
      hotCourses,
      // 如果需要在模板中使用 isLoading 狀態來顯示骨架屏或空狀態，需要返回它
      isLoading,
    };
  },
  // 如果您在 setup 中沒有使用 <script setup> 而是使用了 options API 風格，
  // 並且在 methods, data, computed 中定義了 sidebar 相關屬性，也需要在此移除。
  // 根據您提供的代碼，您使用的是 setup 函數，所以上面的修改應該足夠。
};
</script>
...
.hero-overlay {
  ...
  width: 100%;
  height: 100%;
  background: linear-gradient(rgba(17, 24, 39, 0.8), rgba(194, 255, 235, 0.4));
}
  ...
  .btn-secondary {
    ...
    &:hover {
      background-color: rgba(
        ...
        131,
        0.1
      ) !important; /* hover 時的微弱背景 */
      color: #ffffff !important; /* hover 時稍微變深的文字顏色 */
    }
  }
}

/* 從 123.txt 複製過來的課程卡片網格樣式 */
.courses-section {
  /* 保留原有的 courses-section 樣式 */
  padding: 5rem 0;
  background-color: var(--bg-primary);
  text-align: left;
  ...
  .course-card.new-style-card {
    ...
    .course-image {
      height: 200px; /* 圖片容器高度，與 123.txt 一致 */
      overflow: hidden;

      img {
        width: 100%;
        height: 100%;
        object-fit: cover; /* 確保圖片覆蓋容器並保持比例 */
    ...
    .course-content {
      padding: 1.5rem; /* 內部內邊距 */
      display: flex;
      flex-direction: column;
      flex-grow: 1; /* 讓內容區塊佔滿剩餘空間，保持按鈕在底部 */

      h3 {
        font-size: 1.2rem;
        margin-bottom: 0.5rem;
        color: var(--highlight-color); /* 標題顏色 */
        text-align: left;
      }

      .course-info {
        display: flex;
        gap: 1rem; /* info 項目之間的間距 */
        color: var(--text-secondary); /* info 文字顏色 */
        font-size: 0.9rem; /* info 字體大小 */
        margin-bottom: 1rem; /* info 區塊與按鈕間距 */
        text-align: left;

        .el-icon {
          margin-right: 0.3rem;
          color: var(--highlight-color); /* icon 顏色 */
        }
      }

      /* 查看詳情按鈕樣式 */
      .el-button {
        align-self: flex-start; /* 左對齊按鈕 */
  ...
  .courses-section .view-all-courses .el-button {
    color: var(--highlight-color);
    /* 加入或修改這一行，並加上 !important */
    font-size: 1.2rem !important; /* 您可以根據需要調整這個值 */
  }

  .courses-section .view-all-courses .el-button:hover {
    color: var(--highlight-hover);
  }

  .courses-section .view-all-courses .el-button .el-icon {
    margin-left: 0.5rem;
  }
...
.view-all-courses .view-all-courses-link {
  color: var(--highlight-color); /* 設定連結顏色 */
  text-decoration: none; /* 移除預設下劃線 */
  font-size: 1.2rem; /* 設定您想要的字體大小 */
  display: inline-flex; /* 讓內容（文字和圖標）水平排列 */
  align-items: center; /* 垂直居中對齊內容 */
  transition: color 0.3s ease; /* 添加顏色過渡效果 */
}

.view-all-courses .view-all-courses-link:hover {
  color: var(--highlight-hover); /* 設定 hover 時的顏色 */
  text-decoration: underline; /* 可選：hover 時顯示下劃線 */
}

.view-all-courses .view-all-courses-link .el-icon {
  margin-left: 0.5rem; /* 保持圖標和文字的間距 */
  /* 圖標顏色通常會繼承父元素的 color */
}

/* HomePage 熱門課程區塊的響應式設計 (複製 123.txt 的響應式部分並調整) */
@media (max-width: 992px) {
  ...
  /* 平板下調整卡片內容內邊距 */
  .courses-section .course-card.new-style-card .course-content {
    padding: 1rem;
  }
}
...
@media (max-width: 576px) {
  ...
  /* 微調卡片內部的字體和間距 */
  .courses-section .course-card.new-style-card .course-content h3 {
    font-size: 1.1rem;
  }
  .courses-section .course-card.new-style-card .course-content .course-info {
    font-size: 0.85rem;
    gap: 0.8rem;
  }
  .courses-section .course-card.new-style-card .course-content .el-button {
    padding: 0.4rem 1rem;
    font-size: 0.9rem;
  }
}
